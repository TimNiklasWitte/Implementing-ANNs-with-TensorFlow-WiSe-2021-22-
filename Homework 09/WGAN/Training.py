...
def main():
    ...
    with summary_writer.as_default():
        ...
        train_size = 140000 # Total size: 141545
        test_size = 350
        ...
        test_loss_g, test_loss_c  = wgan.test(test_dataset)
        tf.summary.scalar(name="Test loss generator", data=test_loss_g, step=0)
        tf.summary.scalar(name="Test loss critic", data=test_loss_c, step=0)


        # Check how model performs on train data once before we begin
        train_loss_g, train_loss_c = wgan.test(train_dataset.take(200)) # approx 
        tf.summary.scalar(name="Train loss generator", data=train_loss_g, step=0)
        tf.summary.scalar(name="Train loss critic", data=train_loss_c, step=0)
        ...
        fake_images = wgan.G(noise, training=False)
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for data in tqdm.tqdm(train_dataset,position=0, leave=True, total=train_size/32):
                g_loss, c_loss = wgan.train_step(data)

                epoch_g_loss_agg.append(g_loss)
                epoch_c_loss_agg.append(c_loss)
                
            train_loss_g = np.mean(epoch_g_loss_agg)
            train_loss_c = np.mean(epoch_c_loss_agg)
            
            tf.summary.scalar(name="Train loss generator", data=test_loss_g, step=epoch + 1)
            tf.summary.scalar(name="Train loss critic", data=test_loss_c, step=epoch + 1)
            ...
            tf.summary.scalar(name="Test loss critic", data=test_loss_c, step=epoch + 1)
    ...
    plt.suptitle("Generated candles by applying a WGAN", fontsize=22)
    plt.tight_layout()
